document.addEventListener("DOMContentLoaded", function () {
    ...
    function aplicarMetodoPagoVisualmente() {
        ...
        // Limpiar selección previa de todas las imágenes
        document.querySelectorAll('.boton-metodo img').forEach(img => {
            img.classList.remove('seleccionado');
        });

        // Aplicar clase seleccionado a la imagen correspondiente
        const imagenActiva = document.querySelector(`.boton-metodo img[data-metodo="${metodoGuardado}"]`);
        if (imagenActiva) {
            imagenActiva.classList.add('seleccionado');
        }
    }
    ...
    form.addEventListener('submit', function(e) {
        ...
        const costoEnvio = parseInt(precioEnvioElement?.dataset?.precio || 0);

        // Validar que el costo de envío sea válido
        if (costoEnvio <= 0) {
            alert('Error: No se pudo obtener el costo de envío. Por favor selecciona una ciudad válida.');
            return;
        }
    ...
    // Función para mostrar modal de confirmación (solo para Wompi)
    function mostrarModalConfirmacion(data, costoEnvio) {
        // Solo mostrar modal para Wompi, Nequi va directo al pago
        if (data.metodo_pago !== 'wompi') {
            console.log('💳 Método de pago no es Wompi, saltando modal y procesando pago directamente');
    ...
    // Asignar event listeners para métodos de pago
    document.querySelectorAll('.boton-metodo img').forEach(img => {
    ...
    // Función para redirigir a Wompi checkout
    function crearWidgetWompi(respuesta, data) {
        console.log('🔄 Creando checkout de Wompi...');
        
        // Crear la URL de redirección a Wompi usando el método de enlace directo
        const wompiUrl = `https://checkout.wompi.co/l/${respuesta.public_key}?` + 
            `amount=${respuesta.amount * 100}&` +
            `currency=COP&` +
            `reference=${respuesta.reference}&` +
            `customerEmail=${encodeURIComponent(data.correo || 'cliente@finoso.com')}&` +
            `customerData[fullName]=${encodeURIComponent(data.nombre)}&` +
            `customerData[phoneNumber]=${encodeURIComponent(data.celular)}&` +
            `shippingAddress[addressLine1]=${encodeURIComponent(data.direccion)}&` +
            `shippingAddress[city]=${encodeURIComponent(data.ciudad)}&` +
            `shippingAddress[region]=${encodeURIComponent(data.departamento)}&` +
            `shippingAddress[country]=CO&` +
            `redirectUrl=${encodeURIComponent(respuesta.transaction_data.redirect_url)}`;
        
        console.log('🔄 URL de Wompi generada:', wompiUrl);
