...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function manejarClickComprar(e) {
        ...
        // Detectar origen de compra
        const origenCompra = sessionStorage.getItem('origen_compra');
        console.log("⭐ Origen de compra:", origenCompra);
        
        // Función para procesar el pago con los datos de productos
        const procesarPago = (productos, totalPago) => {
            ...
            fetch(`https://finoso.store/informacion/php/obtener_ciudades.php?action=ciudades&departamento=${departamentoVal}`)
                ...
                .then(ciudadesData => {
                    ...
                    if (ciudadesData.success && ciudadesData.ciudades) {
                        const ciudadSeleccionada = ciudadesData.ciudades.find(ciudad => ciudad.ciudad === ciudadVal);
                    ...
                    console.log("✅ Datos de pago preparados:", datosPago);
                    ...
                    if (metodoPagoVal === "nequi") {
                        console.log("💳 Redirigiendo a Nequi");
                        window.location.href = "pago_nequi-carrito.html";
                    } else if (metodoPagoVal === "wompi") {
                        console.log("💳 Procesando con Wompi");
                        // Enviar datos completos para Wompi
                        procesarPagoWompi({
                            productos: productos,
                            total: totalPago,
                            costo_envio: costoEnvio,
                ...
                .catch(error => {
                    ...
                    if (metodoPagoVal === "nequi") {
                        ...
                        // Enviar a Wompi con costo de envío 0
                        procesarPagoWompi({
                            productos: productos,
                            total: totalPago,
                            costo_envio: 0,
        ...
        // Obtener productos según el origen
        if (origenCompra === 'favoritos') {
            console.log("⭐ Cargando desde FAVORITOS (localStorage)");
            
            // Usar los productos ya cargados en window.currentProducts
            if (window.currentProducts && window.currentProducts.length > 0) {
                console.log("✅ Productos encontrados en window.currentProducts:", window.currentProducts);
                ...
                console.log("✅ Total calculado desde favoritos:", total);
                
                // Procesar pago
                procesarPago(productos, total);
            } else {
                console.error("❌ No hay productos en window.currentProducts");
            ...
            fetch('https://finoso.store/php/mostrar_carrito.php')
                ...
                .then(data => {
                    ...
                    if (data.success && data.relojes && data.relojes.length > 0) {
                        ...
                        // Procesar pago
                        procesarPago(productos, data.total);
                    } else {
