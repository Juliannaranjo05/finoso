...
function enviarCorreoVerificacion($emailDestino, $token) {
    ...
    try {
        ...
        $mail->isHTML(true);
        $mail->CharSet = 'UTF-8';
        $mail->Subject = '✨ Verifica tu cuenta en FINOSO';
                            ...
                            <tr>
                                <td style="padding: 50px 40px; text-align: center;">
                                    ...
                                    <p style="margin: 0 0 30px; color: #e0e0e0; font-size: 16px; line-height: 1.6;">
                                        Estás a un paso de acceder a nuestra exclusiva colección de relojes de lujo.<br>
                                    ...
                                    <table role="presentation" style="margin: 0 auto;">
                                        <tr>
                                            <td style="border-radius: 50px; background: linear-gradient(135deg, #FFCF66 0%, #d4a537 100%); box-shadow: 0 5px 25px rgba(255, 207, 102, 0.4);">
                                                <a href="http://localhost/finoso/login/registrarse/php/verificar.php?token=' . $token . '" 
                                    ...
                                    <p style="margin: 35px 0 0; color: #999; font-size: 13px; line-height: 1.6;">
        ...
        $mail->AltBody = "¡Bienvenido a FINOSO!\n\nPara verificar tu cuenta, copia y pega este enlace en tu navegador:\n\nhttp://localhost/finoso/login/registrarse/php/verificar.php?token=" . $token . "\n\nSi no solicitaste esta cuenta, puedes ignorar este correo.\n\n© 2025 FINOSO - Relojes de Lujo";

        $mail->send();
    } catch (Exception $e) {
        // Log del error en vez de echo (para no romper JSON)
        error_log("Error al enviar correo de verificación: {$mail->ErrorInfo}");
        // No hacemos echo porque rompe el JSON de la respuesta
    }
}
