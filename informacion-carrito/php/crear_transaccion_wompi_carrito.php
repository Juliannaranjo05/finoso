...
wompi_log('=== ➡️ crear_transaccion_wompi_carrito.php INICIO ===');
wompi_log(sprintf('Datos crudos carrito: productos=%d, costo_envio=%d, usuario=%s', count($productos), $costo_envio, $id_usuario ?: '0'));

// Validación básica
if (empty($productos)) {
    wompi_log('❌ No hay productos en el carrito');
...
try {
    ...
    if (empty($productos_validos)) {
        wompi_log('❌ No hay productos válidos en el carrito');
    ...
    if ($stmtCheck->num_rows > 0) {
        ...
        $transactionInProgress = false;
        wompi_log(sprintf('❌ Referencia duplicada detectada (carrito) %s', $referencia));
    ...
    if (!$stmtOrden) {
        mysqli_rollback($conn);
        $transactionInProgress = false;
        wompi_log('❌ (Carrito) No fue posible preparar la inserción de la orden: ' . $conn->error);
    ...
    if (!$stmtOrden->execute()) {
        ...
        mysqli_rollback($conn);
        $transactionInProgress = false;
        wompi_log('❌ (Carrito) Error al guardar orden: ' . $stmtOrden->error);
    ...
    if (!$stmtDetalle) {
        mysqli_rollback($conn);
        $transactionInProgress = false;
        wompi_log('❌ (Carrito) No fue posible preparar detalle: ' . $conn->error);
    ...
    foreach ($productos_validos as $producto_valido) {
        ...
        if (!$stmtDetalle->execute()) {
            ...
            mysqli_rollback($conn);
            $transactionInProgress = false;
            wompi_log('❌ (Carrito) Error al guardar detalle: ' . $stmtDetalle->error);
    ...
    wompi_log('Signature String (carrito): ' . $signature_string);
    wompi_log('Signature Hash (carrito): ' . $signature);
    ...
    wompi_log('=== WOMPI CARRITO CHECKOUT DIRECTO ===');
    wompi_log('Carrito: ' . $carrito_items_count . ' reloj(es)');
    ...
    wompi_log('❌ Excepción (carrito) no controlada: ' . $e->getMessage());
    echo json_encode([
        'error' => 'Error al crear transacción',
        'message' => $e->getMessage()
    ]);
}
wompi_log('=== ⬅️ crear_transaccion_wompi_carrito.php FIN ===');
?>
